...
Por registro:
    ...
    3. PTB-XL (500 Hz) se resamplea a 400 Hz con resample_poly (factor exacto 4/5). Los
       otros dos ya estan a 400 Hz nativo.
    4. Se recorta una ventana de 7.0s = 2.800 muestras, CENTRADA sobre la señal real (no
    ...
    5. Z-score por registro y por derivacion (no global): elimina diferencia de escala
...
def procesar_registro(señal: np.ndarray, frecuencia_nativa: int) -> np.ndarray | None:
    ...
    n = tramo.shape[0]
    if n < WINDOW_SAMPLES:
        return None  # margen de seguridad ante redondeo del resampleo, no deberia pasar
...
def main():
    ...
    filas_out = []
    descartados = {"code15": 0, "samitrop": 0, "ptbxl": 0}
    contador = 0
        ...
        for (dataset, source_file), grupo in tqdm(meta.groupby(["dataset", "source_file"]), desc="archivos"):
            ...
            with h5py.File(path, "r") as fin:
                ...
                for idx, row in grupo.iterrows():
                    ...
                    procesada = procesar_registro(señal, row["frecuencia"])
                    if procesada is None:
                        descartados[dataset] += 1
                        continue
    ...
    print(f"Descartados por <{WINDOW_SEC}s de señal real: {descartados} (total {sum(descartados.values())})")
    print("\nPor split y dataset:")
